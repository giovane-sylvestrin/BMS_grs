...
float x_prevA = 0; //Estimação prévia do SOC Proposta A
float x_prevB = 0; //Estimação prévia do SOC Proposta B
float x_prev = 0;
...
float x_0A = 100;  //Definição do SOC inicial e variável auxiliar na estimação SOC Proposta A
float x_0B = 100;  //Definição do SOC inicial e variável auxiliar na estimação SOC Proposta A
...
float P_prevA = 0; //Covariância do erro de estimação prévia Proposta A
float P_prevB = 0; //Covariância do erro de estimação prévia Proposta B
float P_0A = 10000; //Covariância do erro de estimação inicial e atualizado
float P_0B = 10000; //Covariância do erro de estimação inicial e atualizado
float y_prevA = 0; //Tensão terminal estimada Proposta A
float y_prevB = 0; //Tensão terminal estimada Proposta B
float x_posA = 0;  //SOC estimado atualizado Proposta A
float x_posB = 0;  //SOC estimado atualizado Proposta B
float x_posmxA = 0;  //Limite superior de estimação SOC (intervalo de confiança) Proposta A
float x_posmxB = 0;  //Limite superior de estimação SOC (intervalo de confiança) Proposta B
float x_posmiA = 0;  //Limite inferior de estimação SOC (intervalo de confiança) Proposta A
float x_posmiB = 0;  //Limite inferior de estimação SOC (intervalo de confiança) Proposta B
...
int ref = 0;  //Variável auxiliar de contagem da proposta B
...
void monitoramento(time_t t){
  ...
  Serial.print(x_posA);
  Serial.print(",\"x_mxA\":");
  Serial.print(x_posmxA);
  Serial.print(",\"x_mnA\":");
  Serial.print(x_posmiA);
  Serial.print(",\"x_posB\":");
  Serial.print(x_posB);
  Serial.print(",\"x_mxB\":");
  Serial.print(x_posmxB);
  Serial.print(",\"x_mnB\":");
  Serial.print(x_posmiB);
  Serial.print(",\"x_cc\":");
  Serial.print(x_cc);
  Serial.print(",\"DOD_d\":");
  Serial.print(DOD);
  ...
  Serial.print(DODc);
  Serial.print(",\"y_ekfA\":");
  Serial.print(y_prevA,4);
  Serial.print(",\"y_ekfB\":");
  Serial.print(y_prevB,4);
  ...
  file.print(v_0,4);
  file.print(" VekfA: ");
  file.print(y_prevA,4);
  file.print(" SOCA: ");
  file.print(x_posA,4);
  file.print(" ");
  file.print(x_posmxA,4);
  file.print(" ");
  file.print(x_posmiA,4);
  file.print(" VekfB: ");
  file.print(y_prevB,4);
  file.print(" SOCB: ");
  file.print(x_posB,4);
  file.print(" ");
  file.print(x_posmxB,4);
  file.print(" ");
  file.print(x_posmiB,4);
  file.print(" SOCcc: ");
  file.print(x_cc,4);
  file.print(" DODd (Ah): ");
  file.print(DOD);
...
void EKF(){
    ...
    //ref indica se SOC da bateria esta sendo calculado em:
    //carga (0), repouso (1), EKF (2), EKF proposta B (3)
    //Valores são gravados no cartão SD para análise posterior, definindo o % de estimação usando proposta B
    if(corrente == 0)
    {
      ref = 1;   
    }else{
    ...
    if(x_0A = 100) //Se SOC inicial for 100%, EKF esta habilitado
    {
      estado = 1;
    }

    if((corrente == 0) && (x_0A<100)) //Se SOC inicial for 100%, EKF esta habilitado
    {
      estado = 0;
    }
    ...
    x_prevA = x_0A - ((abs(t_end - t_0)+abs(t_0 - t_end1))/3.6e06)*(corrente/38)*100;
    x_prevB = x_0B - ((abs(t_end - t_0)+abs(t_0 - t_end1))/3.6e06)*(corrente/38)*100;
    ...
    if((st==0) && (corrente>0)) //Redefine parâmetros após carga
    {
      ...
      P_0A = 10000; //Todo o inicio de descarga a covariancia do erro é reiniciada no maximo
      P_0B = 10000;
    ...
    C = C2-C1*corrente;
    //-------------------------------------------------------------------------------------------------------------------
    y_prevA = ocv - R_in*corrente; //Tensão terminal estimada a partir do SOC prévio
    
    //Alterações da Proposta A quando às covariãncias Q e R em função do desvio de tensão do modelo
    if((corrente>0)&&(abs(v_0-y_prevA)>0.002)&&(P_0A<1))
    {
      Q = 0.0001;
      R = 0.01;

      if(abs(v_0-y_prevA)>0.01)
      {
        R = 0.1;
      } 
    }else{
    ...
    P_prevA = 1*P_0A*1+Q; //Covariância do erro na estimativa a priori do estado
    
    L = (P_prevA*C)/(C*P_prevA*C+R); //Ganho de Kalman

    //Atualização do estado estimado   
    x_posA = x_prevA + ((P_prevA*C)/(C*P_prevA*C+R))*(v_0 - y_prevA);
    ...
    P_0A = (1-L*C)*P_prevA*(1-L*C)+L*R*L;

    //Alteração proposta A
    if((corrente>0)&&(abs(v_0-y_prevA)>0.25))
    {
      P_0A = 10000;
    }
    ...
    x_posmxA = x_posA + 10*sqrt(P_0A);
    x_posmiA = x_posA - 10*sqrt(P_0A);  
    ...
    C = C2-C1*corrente;
    //-------------------------------------------------------------------------------------------------------------------
    y_prevB = ocv - R_in*corrente; //Tensão terminal estimada a partir do SOC prévio
                
    P_prevB = 1*P_0B*1+Q; //Covariância do erro na estimativa a priori do estado
    
    L = (P_prevB*C)/(C*P_prevB*C+R); //Ganho de Kalman

    //Atualização do estado estimado   
    x_posB = x_prevB + ((P_prevB*C)/(C*P_prevB*C+R))*(v_0 - y_prevB);

    //Alteração proposta B: 
    //Se a tensão estimada desviar da medida, atualização do SOC é desconsiderada
    if((corrente>0)&&(abs(v_0-y_prevB)>0.005))
    {
      x_posB = x_prevB;
      ref = 3; //Indica que o SOC EKF está sendo alerado pela proposta B
    }
    ...
    //Atualização da covariância do erro de estimativa
    P_0B = (1-L*C)*P_prevB*(1-L*C)+L*R*L;
       
    //Cálcula intervalo de confiança e adiciona à estimativa EKF 
    x_posmxB = x_posB + 10*sqrt(P_0B);
    x_posmiB = x_posB - 10*sqrt(P_0B);  
...
void Coulomb_carga(){
     ref = 0;
     t_end = millis();
     //Atualiza variáveis do SOC: EKF Proposta A/B, contagem de Coulomb
     x_posA = x_0A - 1*((abs(t_end - t_0)+abs(t_0 - t_end1))/3.6e06)*(corrente/38)*100;
     x_posB = x_0B - 1*((abs(t_end - t_0)+abs(t_0 - t_end1))/3.6e06)*(corrente/38)*100;
